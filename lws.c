...
/* LOG_PERROR is not POSIX standard, and may not be portable */
        ...
        // connect to redis context
        // if this connection fails and the server needs to reconnect 
        // while the server is running then you need to use server_lock function
        ...
        while (n >= 0 && !force_exit) {
                ...
                // check redis, if there is anything that is not expired, any message in redis,
                // broadcast message to every websocket connection to take a look
                // remove time part etc
