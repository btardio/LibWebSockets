/*
 *
 * If you ever install this on the server, don't use DNF, install from source
 * 
 */
#include "lwsev.h"
...
//         },
        {
                "acrylic-process-protocol",
                callback_acrylic_process,
                sizeof(struct per_session_data__acrylic_process),
                128, //10,
        },
//         {
//                 "lws-mirror-protocol",
...
ev_timeout_cb (EV_P_ ev_timer *w, int revents)
{
  ...
  lws_callback_on_writable_all_protocol(context,
                                        &protocols[PROTOCOL_ACRYLIC_PROCESS]);
}
...
void rConnectCallback(const redisAsyncContext *c, int status) {
...
void rDisconnectCallback( const redisAsyncContext *c, int status ){
...
int main(int argc, char **argv)
{
        ...
        info.vhost_name = "brandontardio.com";
        ...
        // TODO: write a function that removes set entries from redis, otherwise sets will become huge

        //_ev_timer_init(&timeout_redis, ev_timeout_redis_cb, 0.30, 0.30);
        //ev_timer_start(loop, &timeout_redis);
        ...
        redis_async_context = redisAsyncConnect("127.0.0.1", 6379);
        if (redis_async_context->err) {
          printf("(HI)Redis Error: %s\n", redis_async_context->errstr);
          // handle error
        }
        
        redisLibevAttach(loop, redis_async_context);
        
        redisAsyncSetConnectCallback(redis_async_context, rConnectCallback);
        redisAsyncSetDisconnectCallback(redis_async_context, rDisconnectCallback);
